"""
EB Property Comparison: Thresholding Effects on Lifetime, Area, and Contrast
Analysis of how thresholding affects Ellerman Bomb properties across multiple datasets.
"""
...
# ======================= CONFIGURATION =======================
class Config:
    BASE_PATH = 'data'  # Update to your data path
    OUTPUT_PATH = 'results'
...
def analyze_datasets():
    ...
    for dataset in Config.DATASETS:
        ...
        # Analyze each EB
        for eb_id in np.unique(sources["id"]):
...
def print_statistics(results):
    ...
    print(f"\nArea (Mm²):")
    print(f"All: {len(results['areas']['mm_all'])}, "
          f"Median: {np.median(results['areas']['mm_all']):.3f}")
    print(f"Thresh: {len(results['areas']['mm_thresh'])}, "
          f"Median: {np.median(results['areas']['mm_thresh']):.3f}")
